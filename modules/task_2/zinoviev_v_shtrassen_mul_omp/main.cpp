...
double* matrix_mul(double* A, double* B, int n, double* time) {
  int size = n*n;
  double * C = new double[size];
  ...
  *time += omp_get_wtime() - tmp;
  return C;
}

inline double* matrix_sum_omp(double* A, double* B, int size) {
  double * C = new double[size];
...
inline double* matrix_sub_omp(double* A, double* B, int size) {
  double * C = new double[size];
#pragma omp parallel for
  for (int i = 0; i < size; ++i) {
    C[i] = A[i] - B[i];
...
double* matrix_mul_omp(double* A, double* B, int n, double* time) {
  int size = n * n;
  double * C = new double[size];
  double tmp = omp_get_wtime();
#pragma omp parallel for
...
double* shtrassen_mul_omp(double* A, double* B, int n, double* time) {
	if (n > 64) {
...
#pragma omp parallel sections
    {
#pragma omp section
      P[0] = shtrassen_mul_omp(matrix_sum_omp(A_[0], A_[3], size_4),
        matrix_sum_omp(B_[0], B_[3], size_4), n_2, time);
#pragma omp section
      P[1] = shtrassen_mul_omp(matrix_sum_omp(A_[2], A_[3], size_4),
        B_[0], n_2, time);
#pragma omp section
      P[2] = shtrassen_mul_omp(A_[0],
        matrix_sub_omp(B_[1], B_[3], size_4), n_2, time);
#pragma omp section
      P[3] = shtrassen_mul_omp(A_[3],
        matrix_sub_omp(B_[2], B_[0], size_4), n_2, time);
#pragma omp section
      P[4] = shtrassen_mul_omp(matrix_sum_omp(A_[0], A_[1], size_4),
        B_[3], n_2, time);
#pragma omp section
      P[5] = shtrassen_mul_omp(matrix_sub_omp(A_[2], A_[0], size_4),
        matrix_sum_omp(B_[0], B_[1], size_4), n_2, time);
#pragma omp section
      P[6] = shtrassen_mul_omp(matrix_sub_omp(A_[1], A_[3], size_4),
        matrix_sum_omp(B_[2], B_[3], size_4), n_2, time);
    }
    *time += omp_get_wtime() - tmp;
...
#pragma omp parallel for
    ...
    *time += omp_get_wtime() - tmp;
		...
		return matrix_mul_omp(A, B, n, time);
	}
}

double* shtrassen_mul(double* A, double* B, int n, double* time) {
  if (n > 64) {
    int size = n*n, n_2 = n / 2, size_4 = size / 4;
    double ** A_ = new double*[4];
    double ** B_ = new double*[4];
    ...
    return matrix_mul(A, B, n, time);
  }
}

double* expand_shtrassen_omp(double* A, double* B, int n, double* time) {
  if (n > 64) {
    int check_pow[8] = { 128, 256, 512, 1024, 2048, 4096, 8192, 16384 };
    int new_size = 0;
    for (int i = 0; i < n; ++i) {
      if (check_pow[i] == n) {
        return shtrassen_mul(A, B, n, time);
      }
      if (n < check_pow[i]) {
...
#pragma omp parallel for
    ...
    res_s = shtrassen_mul_omp(new_A, new_B, new_size, time);
...
double* expand_shtrassen(double* A, double* B, int n, double* time) {
  if (n > 64) {
    int check_pow[8] = { 128, 256, 512, 1024, 2048, 4096, 8192, 16384 };
    int new_size = 0;
    for (int i = 0; i < n; ++i) {
    ...
    double* res_s = nullptr;
		...
		res_s = shtrassen_mul(new_A, new_B, new_size, time);
...
int main(int argc, char** argv) {
  ...
  res_mul = matrix_mul(A, B, n, &mul_t);
  std::cout << "done!" << std::endl << std::endl;
  std::cout << "shtrassen multiplicating..." << std::endl;
  res_shtrass_seq = expand_shtrassen(A, B, n, &shtrass_seq_t);
  std::cout << "done!" << std::endl << std::endl;
  std::cout << "shtrassen omp parallel multiplicating..." << std::endl;
  res_shtrass_omp = expand_shtrassen_omp(A, B, n, &shtrass_omp_t);
  std::cout << "done!" << std::endl << std::endl;
  if (n < 10) {
